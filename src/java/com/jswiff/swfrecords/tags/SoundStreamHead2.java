...
public final class SoundStreamHead2 extends Tag {
   ...
   * sampling rate (use <code>RATE_...</code> constants), specify whether the
   * sample size is 16 bit or 8 bit, and provide the channel count (mono /
   * stereo) and the average number of samples (for stereo sound: sample pairs)
   * per SoundStreamBlock.
   * </p>
   * 
   * <p>
   * The advisory playback parameters (sampling rate, sample size and channel
   * count) are set to be identical to the stream's parameters specified here.
   * Use <code>setPlayback...()</code> methods for changing these values.
   * </p>
   * 
   * @param format
   ...
   *          sampling rate
   * @param is16BitSample
   *          if <code>true</code>, sample size is 16 bit, otherwise 8 bit
   * @param isStereo
  ...
  public SoundStreamHead2(byte format, byte rate, boolean is16BitSample, boolean isStereo, int sampleCount) {
    super(TagType.SOUND_STREAM_HEAD_2);
   ...
   * 
   * @param is16BitSample
   *          <code>true</code> for 16 bit, <code>false</code> for 8 bit
   */
   ...
   * 
   * @param isStereo
   *          <code>true</code> for stereo, <code>false</code> for mono
   */
